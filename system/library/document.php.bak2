	...
	private $scripts = array();
		...
		public function getOctStyles($position = 'header') {
			if (isset($this->octStyles[$position])) {
				$styles = isset($this->styles[$position]) ? $this->styles[$position] : [];
				return array_merge($this->octStyles[$position], $styles);
			} else {
				return [];
			}
		}
		...
		public function getOctScripts($position = 'header') {
			if (isset($this->octScripts[$position])) {
				...
				return array_merge($this->octScripts[$position], $scripts);
			} else {
				return array();
			}
		}
		...
		private function removeVersion($link) {
			$href = explode('?', $link);
			if (isset($href[0]) && file_exists($_SERVER['DOCUMENT_ROOT'] . '/' . ltrim($href[0], '/'))) {
				$link = $href[0];
			}
			return $link;
		}
