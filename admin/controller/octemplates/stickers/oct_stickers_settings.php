...
class ControllerOCTemplatesStickersOCTStickersSettings extends Controller {
	...
	private function generateCss($data) {
		...
		if (isset($data['oct_stickers_status']) && $data['oct_stickers_status']) {
			if (isset($data['oct_stickers_data']['stickers'])) {
				foreach ($data['oct_stickers_data']['stickers'] as $key => $stickers) {
					if (isset($stickers['status']) && $stickers['status']) {
						$text_color = $stickers['text_color'] ? preg_replace('/[^a-zA-Z0-9#(),.\s%]/', '', $stickers['text_color']) : "#fff";
						$fon_color = $stickers['fon_color'] ? preg_replace('/[^a-zA-Z0-9#(),.\s%]/', '', $stickers['fon_color']) : "#000";
						$key_safe = preg_replace('/[^a-zA-Z0-9_-]/', '', $key);

						$css .= '.ds-module-sticker-stickers_'. $key_safe .' {color:'. $text_color .';background:'. $fon_color .';}';
					}
				}
			}
			...
			if (isset($data['oct_stickers_data']['customer'])) {
				foreach ($data['oct_stickers_data']['customer'] as $key_customer => $customer) {
					if (isset($customer['status']) && $customer['status']) {
						$text_color = $customer['text_color'] ? preg_replace('/[^a-zA-Z0-9#(),.\s%]/', '', $customer['text_color']) : "#fff";
						$fon_color = $customer['fon_color'] ? preg_replace('/[^a-zA-Z0-9#(),.\s%]/', '', $customer['fon_color']) : "#000";
						$key_customer_safe = preg_replace('/[^a-zA-Z0-9_-]/', '', $key_customer);

						$css .= '.ds-module-sticker-stickers_'. $key_customer_safe .' {color:'. $text_color .';background:'. $fon_color .';}';
					}
				}
			}
			...
			$cache_dir = DIR_CATALOG . '../min/cache/';
			if (is_dir($cache_dir)) {
				$this->delTree($cache_dir);
			}
		}
	}
