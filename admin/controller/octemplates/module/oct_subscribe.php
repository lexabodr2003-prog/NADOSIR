...
class ControllerOCTemplatesModuleOctSubscribe extends Controller {
    ...
    public function index() {
        ...
        if (isset($this->request->get['filter_approved'])) {
            $url .= '&filter_approved=' . (int)$this->request->get['filter_approved'];
        }

        if (isset($this->request->get['filter_ip'])) {
            $url .= '&filter_ip=' . urlencode($this->request->get['filter_ip']);
        }
        ...
        foreach ($results as $result) {
            if (!$result['approved']) {
                $approve = $this->url->link('octemplates/module/oct_subscribe/approve', 'user_token=' . $this->session->data['user_token'] . '&subscribe_id=' . (int)$result['subscribe_id'] . $url, true);
            } else {
                $approve = 1;
            }
    ...
    public function approve() {
        ...
        if ($subscribes && $this->validateApprove()) {
            $this->model_octemplates_module_oct_subscribe->approve((int)$this->request->get['subscribe_id']);
    ...
    public function delete() {
        ...
        if (isset($this->request->post['selected']) && $this->validateDelete()) {
            foreach ($this->request->post['selected'] as $subscribe_id) {
                $this->model_octemplates_module_oct_subscribe->deleteSubscribe((int)$subscribe_id);
            }
