...
class ControllerOctemplatesModuleOctStockNotifier extends Controller {
    ...
    public function index()
    {
        ...
        if (isset($this->request->get['filter_phone'])) {
            $url .= '&filter_phone=' . urlencode($this->request->get['filter_phone']);
        }
        ...
        $filter_email = isset($this->request->get['filter_email']) ? htmlspecialchars($this->request->get['filter_email'], ENT_QUOTES, 'UTF-8') : null;
        $filter_product = isset($this->request->get['filter_product']) ? htmlspecialchars($this->request->get['filter_product'], ENT_QUOTES, 'UTF-8') : null;
        $filter_phone = isset($this->request->get['filter_phone']) ? htmlspecialchars($this->request->get['filter_phone'], ENT_QUOTES, 'UTF-8') : null;
        $filter_status = isset($this->request->get['filter_status']) ? htmlspecialchars($this->request->get['filter_status'], ENT_QUOTES, 'UTF-8') : null;
        ...
        if (isset($data['oct_stock_notifier_data']['cron_secret']) && $data['oct_stock_notifier_data']['cron_secret']) {
            ...
            $data['cron_url'] = $site_link . "index.php?route=octemplates/module/oct_stock_notifier/cron&cron_secret=" . urlencode($data['oct_stock_notifier_data']['cron_secret']);
        }
    ...
    private function getCronUrl($cron_secret) {
        ...
        return $site_link . "index.php?route=octemplates/module/oct_stock_notifier/cron&cron_secret=" . urlencode($cron_secret);
    }
    ...
    protected function validate() {
        ...
        if (isset($this->request->post['oct_stock_notifier_data']['custom_message'])) {
            ...
            foreach ($this->request->post['oct_stock_notifier_data']['message'] as $language_id=>$value) {
                if (!$value || strlen($value) < 30 || strlen($value) > 10000) {
