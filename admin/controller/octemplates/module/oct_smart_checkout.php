...
class ControllerOCTemplatesModuleOctSmartCheckout extends Controller {
    ...
    public function index() {
        ...
        if (is_array($products)) {
            foreach ($products as $product_id) {
                $related_info = $this->model_catalog_product->getProduct((int)$product_id);
    
                if ($related_info) {
                    $data['product_relateds'][] = array(
                        'product_id' => (int)$related_info['product_id'],
                        'name'       => htmlspecialchars($related_info['name'], ENT_QUOTES, 'UTF-8')
                    );
                }
            }
        }
        ...
        foreach ($countries as $country) {
            $data['countries'][] = [
                ...
                'name' => htmlspecialchars($country['name'], ENT_QUOTES, 'UTF-8') . (($country['country_id'] == $this->config->get('config_country_id')) ? $this->language->get('text_default') : null),
            ];
        }
    ...
    public function editCustomFieldForm() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            $custom_field_info = $this->model_customer_custom_field->getCustomField((int)$this->request->post['custom_field_id']);

            $data['custom_field_id'] = (int) $this->request->post['custom_field_id'];

            if (isset($this->request->post['custom_field_id'])) {
                $data['custom_field_description'] = $this->model_customer_custom_field->getCustomFieldDescriptions((int)$this->request->post['custom_field_id']);
            } else {
            ...
            if (!empty($custom_field_info)) {
                ...
                $custom_field_values = $this->model_customer_custom_field->getCustomFieldValueDescriptions((int)$this->request->post['custom_field_id']);
            } 
            ...
            if (isset($this->request->post['custom_field_id'])) {
                $custom_field_customer_groups = $this->model_customer_custom_field->getCustomFieldCustomerGroups((int)$this->request->post['custom_field_id']);
            } else {
    ...
    public function editCustomField() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateCustomFieldForm()) {
            $this->model_customer_custom_field->editCustomField((int)$this->request->post['custom_field_id'], $this->request->post);
    ...
    public function country() {
        ...
        if ($country_info) {
            ...
            $json = [
                ...
                'name' => htmlspecialchars($country_info['name'], ENT_QUOTES, 'UTF-8'),
                'iso_code_2' => htmlspecialchars($country_info['iso_code_2'], ENT_QUOTES, 'UTF-8'),
                'iso_code_3' => htmlspecialchars($country_info['iso_code_3'], ENT_QUOTES, 'UTF-8'),
                'address_format' => htmlspecialchars($country_info['address_format'], ENT_QUOTES, 'UTF-8'),
