...
class ControllerOctemplatesModuleOctReviewReminder extends Controller {
    ...
    public function index() {
        ...
        if (isset($data['oct_review_reminder_cron_password']) && $data['oct_review_reminder_cron_password'] && !isset($this->error['cron_password'])) {
            ...
            $data['cron_url'] = $site_link . "index.php?route=octemplates/module/oct_review_reminder/cron&cron_secret=" . urlencode($data['oct_review_reminder_cron_password']);
        }
    ...
    protected function validate() {
        ...
        if (isset($this->request->post['oct_review_reminder_days_after_order']) && (!is_numeric($this->request->post['oct_review_reminder_days_after_order']) || (int)$this->request->post['oct_review_reminder_days_after_order'] <= 0 || (int)$this->request->post['oct_review_reminder_days_after_order'] > 365)) {
            $this->error['days_after_order'] = $this->language->get('error_days_after_order');
        }

        if (isset($this->request->post['oct_review_reminder_cron_password']) && (strlen($this->request->post['oct_review_reminder_cron_password']) < 5 || strlen($this->request->post['oct_review_reminder_cron_password']) > 128)) {
