...
class ControllerOctemplatesModuleOctOtpLogin extends Controller {
    ...
    protected function validate() {
        ...
        if (isset($this->request->post['oct_otp_login_settings'])) {
            ...
            if (!isset($settings['otp_length']) || !is_numeric($settings['otp_length']) || (int)$settings['otp_length'] < 4 || (int)$settings['otp_length'] > 10) {
                $this->error['otp_length'] = $this->language->get('error_otp_length');
            }

            if (!isset($settings['otp_expiry']) || !is_numeric($settings['otp_expiry']) || (int)$settings['otp_expiry'] <= 0 || (int)$settings['otp_expiry'] > 60) {
                $this->error['otp_expiry'] = $this->language->get('error_otp_expiry');
            }

            if (!isset($settings['max_attempts']) || !is_numeric($settings['max_attempts']) || (int)$settings['max_attempts'] <= 0 || (int)$settings['max_attempts'] > 100) {
                $this->error['max_attempts'] = $this->language->get('error_max_attempts');
            }

            if (!isset($settings['lockout_time']) || !is_numeric($settings['lockout_time']) || (int)$settings['lockout_time'] <= 0 || (int)$settings['lockout_time'] > 1440) {
                $this->error['lockout_time'] = $this->language->get('error_lockout_time');
            }

            if (!isset($settings['throttle_time']) || !is_numeric($settings['throttle_time']) || (int)$settings['throttle_time'] < 30 || (int)$settings['throttle_time'] > 600) {
                $this->error['throttle_time'] = $this->language->get('error_throttle_time');
            }

            if (isset($settings['phone_mask']) && strlen($settings['phone_mask']) > 50) {
                $this->error['phone_mask'] = 'Phone mask is too long (max 50 characters)';
            }
    ...
    public function logs() {
        ...
        foreach ($results as $result) {
            ...
            $email = $customer_info ? htmlspecialchars($customer_info['email'], ENT_QUOTES, 'UTF-8') : '';
            $profile_link = $this->url->link('customer/customer/edit', 'user_token=' . $this->session->data['user_token'] . '&customer_id=' . (int)$result['customer_id'], true);

            $logs[] = [
                'log_id'       => (int)$result['log_id'],
                'customer_id'  => (int)$result['customer_id'],
                'telephone'    => htmlspecialchars($result['telephone'], ENT_QUOTES, 'UTF-8'),
                'email'        => $email,
                'profile_link' => '<a href="' . htmlspecialchars($profile_link, ENT_QUOTES, 'UTF-8') . '" target="_blank" rel="noopener noreferrer">' . $email . '</a>',
                'customer_firstname' => htmlspecialchars($customer_info['firstname'] ?? '', ENT_QUOTES, 'UTF-8'),
                'customer_lastname' => htmlspecialchars($customer_info['lastname'] ?? '', ENT_QUOTES, 'UTF-8'),
    ...
    public function ipLogs() {
        ...
        foreach ($results as $result) {
            $logs[] = [
                ...
                'ip_address'    => htmlspecialchars($result['ip_address'], ENT_QUOTES, 'UTF-8'),
                'attempt_count' => (int)$result['attempt_count'],
                'last_attempt'  => htmlspecialchars($result['last_attempt'], ENT_QUOTES, 'UTF-8')
            ];
        }
