...
class ControllerOctemplatesModuleOctSmsNotify extends Controller {
    ...
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            $settings = $this->request->post['oct_sms_settings'];
            
            if (isset($settings['order_status_items']) && is_array($settings['order_status_items'])) {
                $normalized_items = array();
                foreach ($settings['order_status_items'] as $item) {
                    if (is_array($item)) {
                        if (isset($item['order_status_ids']) && !is_array($item['order_status_ids'])) {
                            $item['order_status_ids'] = array($item['order_status_ids']);
                        }
                        if (!isset($item['order_status_ids'])) {
                            $item['order_status_ids'] = array();
                        }
                        $normalized_items[] = $item;
                    }
                }
                $settings['order_status_items'] = $normalized_items;
            }
            
            if (isset($settings['order_templates']) && is_array($settings['order_templates'])) {
                $normalized_templates = array();
                foreach ($settings['order_templates'] as $template) {
                    if (is_array($template)) {
                        $normalized_templates[] = $template;
                    }
                }
                $settings['order_templates'] = $normalized_templates;
            }
            
            $this->model_setting_setting->editSetting('oct_sms_settings', ['oct_sms_settings' => $settings]);
        ...
        $data['order_status_items'] = isset($data['oct_sms_settings']['order_status_items']) ? $data['oct_sms_settings']['order_status_items'] : array();
    ...
    public function getLogs() {
        ...
        $admin_limit = min(100, max(1, (int)$this->config->get('config_limit_admin')));
        ...
        foreach ($results as $result) {
            $json['logs'][] = array(
            'phone'         => htmlspecialchars($result['phone'], ENT_QUOTES, 'UTF-8'),
            'message'       => htmlspecialchars($result['message'], ENT_QUOTES, 'UTF-8'),
            'provider'      => htmlspecialchars($result['provider'], ENT_QUOTES, 'UTF-8'),
            'date_added'    => htmlspecialchars($result['date_added'], ENT_QUOTES, 'UTF-8'),
            'template_code' => htmlspecialchars($result['template_code'], ENT_QUOTES, 'UTF-8'),
            'response'      => htmlspecialchars($result['response'], ENT_QUOTES, 'UTF-8')
            );
        }
    ...
    protected function validate() {
        ...
        if (empty($this->request->post['oct_sms_settings']['provider'])) {
            ...
            if ($provider == 'turbosms') {
                if (empty($this->request->post['oct_sms_settings']['turbosms']['token']) || strlen(trim($this->request->post['oct_sms_settings']['turbosms']['token'])) < 10 || strlen(trim($this->request->post['oct_sms_settings']['turbosms']['token'])) > 256) {
            ...
            if ($provider == 'alphasms') {
                if (empty($this->request->post['oct_sms_settings']['alphasms']['api_key']) || strlen(trim($this->request->post['oct_sms_settings']['alphasms']['api_key'])) < 5 || strlen(trim($this->request->post['oct_sms_settings']['alphasms']['api_key'])) > 256) {
        ...
        if (isset($this->request->post['oct_sms_settings']['order_status_items'])) {
            foreach ($this->request->post['oct_sms_settings']['order_status_items'] as $key => $item) {
                if (empty($item['system_name'])) {
                    $this->error['order_status'][$key]['system_name'] = $this->language->get('error_system_name');
                }
                
                if (!isset($item['order_status_ids']) || !is_array($item['order_status_ids']) || empty($item['order_status_ids'])) {
                    $this->error['order_status'][$key]['order_status_ids'] = $this->language->get('error_order_status_required');
                }
            }
        }
        ...
        if (empty($this->request->post['oct_sms_settings']['admin_phone']) || strlen(trim($this->request->post['oct_sms_settings']['admin_phone'])) < 9 || strlen(trim($this->request->post['oct_sms_settings']['admin_phone'])) > 20) {
