...
class ControllerOctemplatesModuleOctAbandonedCart extends Controller {
    ...
    public function index() {
        ...
        $pagination->page = isset($this->request->get['page']) ? max(1, (int)$this->request->get['page']) : 1;
        $pagination->limit = 10;
    ...
    public function getLostCarts() {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'GET') {
            ...
            $limit = min(100, max(10, (int)$this->config->get('config_limit_admin')));
            ...
            if ($carts) {
                ...
                foreach ($carts as $cart) {
                    ...
                    if (!empty($cart['ip_added'])) {
                        $ip_added_link = '<b>ip: </b><a href="https://whatismyipaddress.com/ip/' . htmlspecialchars($cart['ip_added'], ENT_QUOTES, 'UTF-8') . '" target="_blank" rel="noopener noreferrer">'
                                    . htmlspecialchars($cart['ip_added'], ENT_QUOTES, 'UTF-8') . '</a>';
                    }

                    $ip_changed_link = '';
                    if (!empty($cart['ip_changed'])) {
                        $ip_changed_link = '<b>ip: </b><a href="https://whatismyipaddress.com/ip/' . htmlspecialchars($cart['ip_changed'], ENT_QUOTES, 'UTF-8') . '" target="_blank" rel="noopener noreferrer">'
                                        . htmlspecialchars($cart['ip_changed'], ENT_QUOTES, 'UTF-8') . '</a>';
                    }
                    ...
                    if (!empty($cart['customer_id'])) {
                        $customer_link = $this->url->link(
                            ...
                            'user_token=' . $this->session->data['user_token'] . '&customer_id=' . (int)$cart['customer_id'],
                            true
                        );
                    }
                    ...
                    $short_id = $this->model_octemplates_module_oct_abandoned_cart->getOrCreateShortLink(
                        $cart['cookie_token'],
                        $cart['cookie_signature'],
                    ...
                    $restore_link = $host . 'index.php?route=octemplates/module/oct_abandoned_cart/restoreCart&r=' . htmlspecialchars($short_id, ENT_QUOTES, 'UTF-8');

                    $data['carts'][] = [
                        'abandoned_cart_id' => (int)$cart['abandoned_cart_id'],
                        'cart_item'         => (int)$cart_item,
                        'firstname'         => htmlspecialchars($cart['firstname'], ENT_QUOTES, 'UTF-8'),
                        'lastname'          => htmlspecialchars($cart['lastname'], ENT_QUOTES, 'UTF-8'),
                        'email'             => htmlspecialchars($cart['email'], ENT_QUOTES, 'UTF-8'),
                        'phone'             => htmlspecialchars($cart['phone'], ENT_QUOTES, 'UTF-8'),
                        'customer_id'       => (int)$cart['customer_id'],
                        'customer_link'     => $customer_link,
                        'products'          => $products,
                        'date_modified'     => htmlspecialchars($cart['date_modified'], ENT_QUOTES, 'UTF-8'),
                        'date_added'        => htmlspecialchars($cart['date_added'], ENT_QUOTES, 'UTF-8'),
                        'last_reminder'     => isset($cart['last_reminder']) ? htmlspecialchars($cart['last_reminder'], ENT_QUOTES, 'UTF-8') : $this->language->get('text_never'),
    ...
    protected function validate() {
        ...
        if (isset($this->request->post['oct_abandoned_cart'])) {
            $settings = $this->request->post['oct_abandoned_cart'];
            
            if (!isset($settings['reminder_hours_first']) || !is_numeric($settings['reminder_hours_first']) || (int)$settings['reminder_hours_first'] < 0 || (int)$settings['reminder_hours_first'] > 720) {
                $this->error['reminder_hours_first'] = $this->language->get('error_complete_all_fields');
            }
            
            if (!isset($settings['reminder_hours_second']) || !is_numeric($settings['reminder_hours_second']) || (int)$settings['reminder_hours_second'] < 0 || (int)$settings['reminder_hours_second'] > 720) {
                $this->error['reminder_hours_second'] = $this->language->get('error_complete_all_fields');
            }
            
            if (!isset($settings['coupon_discount']) || !is_numeric($settings['coupon_discount']) || (int)$settings['coupon_discount'] < 0 || (int)$settings['coupon_discount'] > 99999) {
                $this->error['coupon_discount'] = $this->language->get('error_complete_all_fields');
            }
            
            if (!isset($settings['coupon_lifetime']) || !is_numeric($settings['coupon_lifetime']) || (int)$settings['coupon_lifetime'] < 1 || (int)$settings['coupon_lifetime'] > 365) {
                $this->error['coupon_lifetime'] = $this->language->get('error_complete_all_fields');
            }
            
            if (!isset($settings['cookie_lifetime']) || !is_numeric($settings['cookie_lifetime']) || (int)$settings['cookie_lifetime'] < 1 || (int)$settings['cookie_lifetime'] > 365) {
                $this->error['cookie_lifetime'] = $this->language->get('error_complete_all_fields');
            }
            
            if (!isset($settings['converted_lifetime']) || !is_numeric($settings['converted_lifetime']) || (int)$settings['converted_lifetime'] < 1 || (int)$settings['converted_lifetime'] > 365) {
                $this->error['converted_lifetime'] = $this->language->get('error_complete_all_fields');
            }
            
            if (!isset($settings['coupon_type']) || !in_array($settings['coupon_type'], ['percent', 'fixed'])) {
