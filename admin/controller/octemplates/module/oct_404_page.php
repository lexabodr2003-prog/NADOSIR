...
class ControllerOCTemplatesModuleOct404Page extends Controller {
    ...
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            $this->model_design_layout->editLayout((int)$this->request->post['layout_id'], $this->request->post);
		...
		if (isset($this->request->post['layout_route'])) {
			...
			$data['layout_routes'] = $this->model_design_layout->getLayoutRoutes((int)$layout_id);
		} else {
			$data['layout_routes'] = [];
		}
		...
		if (isset($this->request->post['layout_module'])) {
			...
			$layout_modules = $this->model_design_layout->getLayoutModules((int)$layout_id);
		} else {
			$layout_modules = [];
		}
		...
		foreach ($layout_modules as $layout_module) {
			...
			if (!isset($part[1])) {
				...
				$module_info = $this->model_setting_module->getModule((int)$part[1]);

				if ($module_info) {
					$data['layout_modules'][] = [
						'name'       => strip_tags($module_info['name']),
						'code'       => $layout_module['code'],
						'edit'       => $this->url->link('extension/module/' . $part[0], 'user_token=' . $this->session->data['user_token'] . '&module_id=' . (int)$part[1], true),
						'position'   => $layout_module['position'],
						'sort_order' => $layout_module['sort_order']
					];
				}
