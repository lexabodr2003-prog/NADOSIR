...
class ControllerExtensionFeedYandexYml extends Controller {
    ...
    protected function preparePostData() {
        ...
        if (isset($this->request->post['yandex_yml_custom_tags'])) {
            $custom_tags = array();
            $custom_tag_attrs = array();
            foreach ($this->request->post['yandex_yml_custom_tags'] as $i=>$tag_name) {
                $tag = preg_replace('/[^\w-]/', '', $tag_name);
                if (strlen($tag) > 0) {
                    $custom_tags[] = $tag;
                    $custom_tag_attrs[] = $this->request->post['yandex_yml_custom_tag_attrs'][$i];
                }
            }
            $this->request->post['yandex_yml_custom_tags'] = implode(',', $custom_tags);
            $this->request->post['yandex_yml_custom_tag_attrs'] = implode(',', $custom_tag_attrs);
        }

        if (isset($this->request->post['yandex_yml_vats'])) {
            $this->request->post['yandex_yml_vats'] = serialize($this->request->post['yandex_yml_vats']);
        }
    ...
    protected function setLanguageData() {
        ...
        $this->tdata['breadcrumbs'][] = array(
            'href'      => $this->url->link('extension/feed/yandex_yml', 'user_token=' . $this->session->data['user_token'], 'SSL'),
        ...
        $this->tdata['entry_group_id_attr'] = $this->language->get('entry_group_id_attr');
        ...
        $this->tdata['tab_attributes_description'] = str_replace('%attr_url%', $this->url->link('catalog/attribute', 'user_token=' . $this->session->data['user_token'], 'SSL'), $this->language->get('tab_attributes_description'));
        ...
        $this->tdata['token'] = $this->session->data['user_token'];
    }
    ...
    protected $SETTINGS = array(
        ...
        'utm_label', //UTM-метки
        'group_id_attr', //атрибут group_id
        ...
        'amount', //кол-во для СДЭК.маркет
        'outlet', //кол-во для OZON.ru
        'outlet_name', //Имя склада для OZON.ru
        'quantity_in_stock', //кол-во для prom.ua
        'min_quantity', //минимальный заказ в тэгах min-quantity и step-quantity
		'multioffer', //минимальный заказ в названии товара
    ...
    protected function setFormData() {
        ...
        if (isset($this->request->post['yandex_yml_vats'])) {
            $this->tdata['yandex_yml_vats'] = $this->request->post['yandex_yml_vats'];
        } elseif ($this->config->get($this->CONFIG_PREFIX.'vats') != '') {
            $this->tdata['yandex_yml_vats'] = unserialize($this->config->get($this->CONFIG_PREFIX.'vats'));
        } else {
            $this->tdata['yandex_yml_vats'] = array();
        }
        ...
        if (isset($this->request->post['yandex_yml_custom_tags'])) {
            $this->tdata['yandex_yml_custom_tags'] = $this->request->post['yandex_yml_custom_tags'];
        } elseif ($this->config->get($this->CONFIG_PREFIX.'custom_tags') != '') {
            $this->tdata['yandex_yml_custom_tags'] = explode(',', $this->config->get($this->CONFIG_PREFIX.'custom_tags'));
        } else {
            $this->tdata['yandex_yml_custom_tags'] = array();
        }
        if (isset($this->request->post['yandex_yml_custom_tag_attrs'])) {
            $this->tdata['yandex_yml_custom_tag_attrs'] = $this->request->post['yandex_yml_custom_tag_attrs'];
        } elseif ($this->config->get($this->CONFIG_PREFIX.'custom_tag_attrs') != '') {
            $this->tdata['yandex_yml_custom_tag_attrs'] = explode(',', $this->config->get($this->CONFIG_PREFIX.'custom_tag_attrs'));
        } else {
            $this->tdata['yandex_yml_custom_tag_attrs'] = array();
        }
        //---- Для вкладки аттрибутов ----
    ...
    public function index() {
        ...
        $this->tdata['tax_classes'] = $this->model_localisation_tax_class->getTaxClasses();

        if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate($this->request->post))) {
            $this->preparePostData();

            $this->model_setting_setting->editSetting('feed_yandex_yml', $this->request->post);
        ...
        $this->tdata['action'] = $this->url->link('extension/feed/yandex_yml', 'user_token=' . $this->session->data['user_token'], 'SSL');
        ...
        $template = 'extension/feed/yandex_yml';
