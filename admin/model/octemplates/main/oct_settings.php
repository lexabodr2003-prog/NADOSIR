...
class ModelOCTemplatesMainOctSettings extends Model {
	...
	public function installAuthRateLimit() {
		$sql = "
			CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "oct_auth_rate_limit` (
	...
	public function installShortAbandonedLink() {
		if (!$this->checkIfTableExist(DB_PREFIX . 'oct_abandoned_cart_short_link')) {
			if ($this->checkIfTableExist(DB_PREFIX . 'oct_abandoned_cart')) {
				$this->db->query("DELETE FROM `" . DB_PREFIX . "oct_abandoned_cart`");
			}
		}

		$sql = "
			CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "oct_abandoned_cart_short_link` (
