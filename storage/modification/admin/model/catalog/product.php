		...
		$product_id = $this->db->getLastId();
			...
			if ($this->config->get('oct_stickers_status')) {
				...
				if (isset($data['oct_stickers']) && !empty($data['oct_stickers']) && is_array($data['oct_stickers'])) {
					$oct_stickers_serialized = serialize($data['oct_stickers']);
					$this->db->query("UPDATE " . DB_PREFIX . "product SET oct_stickers = '" . $this->db->escape($oct_stickers_serialized) . "' WHERE product_id = '" . (int)$product_id . "'");
