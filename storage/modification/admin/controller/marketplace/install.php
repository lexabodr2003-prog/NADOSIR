...
class ControllerMarketplaceInstall extends Controller {
	...
	public function move() {
		...
		if (!$json) {
			...
			if ($path == '') {
				$application_path = explode('/', DIR_APPLICATION);
				
				unset($application_path[count($application_path) - 2]);
				
				$application_path = implode('/', $application_path);
							
				$path = $application_path.$destination;
			}
