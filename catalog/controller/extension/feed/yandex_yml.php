 ...
 */
//Версия модуля
define ('YANDEX_YML_VERSION', '1.9.8');
...
//---- Config section ----
    protected $CONFIG_PREFIX = 'feed_yandex_yml_';
    ...
    */
    protected function getYmlHeader() {
        ...
        if ($this->config->get($this->CONFIG_PREFIX.'option_image_pro')) {
            $this->load->model('extension/liveopencart/product_option_image_pro');
            //if ($this->model_extension_liveopencart_product_option_image_pro->installed()) {
        ...
        $supported_currencies = array('RUR', 'RUB', 'USD', 'EUR', 'BYR', 'BYN', 'KZT', 'UAH', 'UAN');
        ...
        $yml .= '<yml_catalog date="' . date('Y-m-d').'T'.date('H:iP') . '">' . $this->eol;
        $yml .= '<shop>' . $this->eol;
        ...
        if ($this->config->get($this->CONFIG_PREFIX.'auto_discounts')) {
            $yml .= '<enable_auto_discounts>yes</enable_auto_discounts>' . $this->eol;
        }
    ...
    protected function getYmlChunk($page = 0) {
        ...
        $yandex_yml_vats = unserialize($this->config->get($this->CONFIG_PREFIX.'vats'));
        ...
        foreach ($this->model_extension_feed_yandex_yml->getProduct($allowed_categories, $blacklist_type, $blacklist, $out_of_stock_ids, $allowed_manufacturers, $this->customer_group, $product_rel, $skip, $limit, $this->is_main_category) as $product) {
            $is_out = false;
            $data = array();
...
//                $data['cbid'] = 15;
            ...
            if (isset($yandex_yml_vats[$product['tax_class_id']]) && $yandex_yml_vats[$product['tax_class_id']]) {
                $data['vat'] = $yandex_yml_vats[$product['tax_class_id']];
            }
            ...
            $data['__product_quantity'] = intval($product['quantity']);
            ...
            */
            ...
            if ($this->config->get($this->CONFIG_PREFIX.'exportattr')) {
                $data['__noexport'] = true;
            }
            $custom_tags = explode(',', $this->config->get($this->CONFIG_PREFIX.'custom_tags'));
            $custom_tag_attrs = explode(',', $this->config->get($this->CONFIG_PREFIX.'custom_tag_attrs'));
            $data['custom_tags'] = array();
            if (count($attributes) > 0) {
                foreach ($attributes as $attr) {
                    ...
                    if (is_array($custom_tags) && is_array($custom_tag_attrs)) {
                        foreach ($custom_tags as $i=>$tag) {
                            if (isset($custom_tag_attrs[$i]) && $custom_tag_attrs[$i] == $attr['attribute_id']) {
                                $data['custom_tags'][$tag] = $attr['text'];
                            }
                        }
                    }
            ...
            if (isset($data['__noexport']) && $data['__noexport']) {
                continue;
            }
            ...
            if ($description_field) {
                if ($description_field == 'attr_vs_description') {
                    $product_description = implode("<br/>\n", $attr_text);
                }
                else {
            ...
            if ($product['minimum'] > 1) {
                ...
                if ($this->config->get($this->CONFIG_PREFIX.'multioffer')) {
					$data['multioffer'] = $product['minimum'];
                }
            ...
            foreach ($this->offers as $idx=>$offer) {
                ...
                if (isset($offer['outlet'])) {
                    $tags .= '<outlets><outlet instock="'.intval($offer['outlet']).'" warehouse_name="'.$this->prepareField($offer['outlet_name']).'"></outlet></outlets>'. $this->eol;
                    unset($offer['outlet']);
                    unset($offer['outlet_name']);
                }
                ...
                if (isset($offer['custom_tags']) && !empty($offer['custom_tags'])) {
                    $tags .= $this->array2Tag($offer['custom_tags']);
                }
                unset($offer['custom_tags']);
    ...
    */
    ...
    protected function setOptionedOffer($data, $product) {
        ...
        if ($this->option_image_pro) {
            $data['picture'] = array();
            $images_by_options = $this->model_extension_liveopencart_product_option_image_pro->getProductOptionImagesByValues($product['product_id']);
        }
        ...
        if (count($coptions)) {
            foreach ($coptions as $option) {
                ...
                if ($option['subtract']) {
                    $data_arr['__product_quantity'] = intval($option['quantity']);
                }

                $data_arr = $this->getOptionedPrices($option, $data_arr);
    ...
    /**
     * Расчёт разных цен с учётов +-ud= в опциях
     */
    protected function getOptionedPrices($option, $data_arr) {
    ...
    */
    ...
    protected function setShop($name, $value) {
        $allowed = array('name', 'company', 'url', 'agency', 'email');
    ...
    protected function setOffer($data) {
        ...
        if ($this->config->get($this->CONFIG_PREFIX.'addprice')) {
            $data['price']+= floatval($this->config->get($this->CONFIG_PREFIX.'addprice'));
            if (isset($data['price_promo'])) {
                $data['price_promo']+= floatval($this->config->get($this->CONFIG_PREFIX.'addprice'));
            }
        }

		if (isset($data['multioffer'])) {
            $data['name'] = $data['name'].', '.$data['multioffer'].'шт.';
			$data['price']*= $data['multioffer'];
            if (isset($data['price_promo'])) {
                $data['price_promo']*= $data['multioffer'];
            }
            if (isset($data['oldprice'])) {
                $data['oldprice']*= $data['multioffer'];
            }
			$data['weight']*= $data['weight'];
			
			unset($data['multioffer']);
		}
        ...
        if ($this->config->get($this->CONFIG_PREFIX.'count')) {
            $data['count'] = $data['__product_quantity'];
        }
        if ($this->config->get($this->CONFIG_PREFIX.'quantity')) {
            $data['quantity'] = $data['__product_quantity'];
        }
        if ($this->config->get($this->CONFIG_PREFIX.'amount')) {
            $data['amount'] = $data['__product_quantity'];
        }
        if ($this->config->get($this->CONFIG_PREFIX.'outlet')) {
            $data['outlet'] = $data['__product_quantity'];
            $data['outlet_name'] = $this->config->get($this->CONFIG_PREFIX.'outlet_name');
        }
        if ($this->config->get($this->CONFIG_PREFIX.'quantity_in_stock')) {
            $data['quantity_in_stock'] = $data['__product_quantity'];
        }
        ...
        $attributes = array('id', 'type', 'available', 'bid', 'cbid', 'param', 'delivery-option', 'accessory', 'age', 'condition', 'opt_prices', 'selling_type', 'outlet', 'outlet_name', 'tn-ved-codes', 'custom_tags');
        if ($this->config->get($this->CONFIG_PREFIX.'group_id_attr')) {
            $attributes[] = 'group_id';
        }
        ...
        $allowed_tags = array('url'=>0, 'buyurl'=>0, 'price'=>1, 'oldprice'=>0, 'price_promo'=>0, 'purchase_price'=>0, 'vat'=>0, 'wprice'=>0, 'currencyId'=>1, 'xCategory'=>0, 'categoryId'=>1, 'picture'=>0, 'store'=>0, 'pickup'=>0, 'delivery'=>0, 'deliveryIncluded'=>0, 'local_delivery_cost'=>0, 'orderingTime'=>0);
        ...
        $allowed_tags = array_merge($allowed_tags, array('aliases'=>0, 'additional'=>0, 'description'=>0, 'sales_notes'=>0, 'promo'=>0, 'manufacturer_warranty'=>0, 'country_of_origin'=>0, 'weight'=>0, 'dimensions'=>0, 'length'=>0, 'width'=>0, 'height'=>0, 'downloadable'=>0, 'adult'=>0, 'age'=>0, 'barcode'=>0, 'keywords'=>0, 'shop-sku'=>0, 'market-sku'=>0, 'manufacturer'=>0, 'rec'=>0, 'count'=>0, 'amount'=>0, 'quantity'=>0, 'quantity_in_stock'=>0, 'min-quantity'=>0, 'step-quantity'=>0, 'minimum_order_quantity'=>0));
    ...
    protected function outYml() {
        if (isset($this->request->get['yml_language'])) {
            $query = $this->db->query("SELECT language_id FROM `" . DB_PREFIX . "language` WHERE code='".$this->db->escape($this->request->get['yml_language'])."'"); 
            if ($query->num_rows) {
                $this->session->data['language'] = $this->request->get['yml_language'];
                $this->config->set('config_language_id', $query->row['language_id']);
                $this->config->set('config_language', $this->request->get['yml_language']);
            }
        }
    ...
    protected function putYml($fp) {
        $options = getopt('l::', array('language::'));
        $language = false;
        if (isset($options['l'])) {
            $language = $options['l'];
        }
        ...
        if ($language) {
            $query = $this->db->query("SELECT language_id FROM `" . DB_PREFIX . "language` WHERE code='".$this->db->escape($language)."'"); 
            if ($query->num_rows) {
    ...
    protected function getCustomerGroup() {
        if ($this->progroman_city_manager) {
            ...
            if ($found) {
                ...
                if (isset($groups[0])) {
                    $ret = $groups[0]['customer_group_id'];
                }
            }
        }
        if (!isset($ret)) {
            $ret = $this->config->get($this->CONFIG_PREFIX.'groupprice');
        }
        if (!$ret) {
            $ret = $this->config->get('config_customer_group_id');
        }
        return $ret;
    }
}
