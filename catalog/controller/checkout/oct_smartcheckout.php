...
class ControllerCheckoutOctSmartcheckout extends Controller {
    public function index() {
        ...
        if (!isset($this->session->data['smart_csrf_token'])) {
            $this->session->data['smart_csrf_token'] = bin2hex(random_bytes(32));
        }
        $data['smart_csrf_token'] = $this->session->data['smart_csrf_token'];
    ...
    public function abandonedSave() {
        ...
        if ($this->request->server['REQUEST_METHOD'] === 'POST') {
			...
			if (empty($token) || empty($session_token) || !hash_equals($session_token, $token)) {
				$this->log->write('[SECURITY] Invalid or missing CSRF token in abandonedSave from IP: ' . $this->request->server['REMOTE_ADDR']);
				return;
			}
		}
    ...
    private function validateCsrf() {
        $token = isset($this->request->post['smart_csrf_token']) ? $this->request->post['smart_csrf_token'] : '';
        $session_token = isset($this->session->data['smart_csrf_token']) ? $this->session->data['smart_csrf_token'] : '';
        ...
        if (isset($this->request->post) && !empty($this->request->post)) {
            ...
            foreach ($data['merged_fields'] as $key => $field) {
                if (isset($field['merge']) && $field['merge'] == 'on' && isset($field['merge_field'])) {
                    ...
                    if (!in_array($mergeField, $allowed_merge_fields)) {
                        continue;
                    }
                    ...
                    if ($field['type'] == 'text' || $field['type'] == 'tel' || $field['type'] == 'textarea' || $field['type'] == 'date' || $field['type'] == 'time' or $field['type'] == 'datetime') {
                        $fieldValue = isset($this->request->post[$key]) ? strip_tags(trim($this->request->post[$key])) : '';
                            ...
                            foreach ($this->request->post[$key] as $selected) {
                                foreach ($field['options'] as $option) {
                                    if ($option['custom_field_value_id'] == $selected) {
                                        $selectedOptions[] = htmlspecialchars($option['name'], ENT_QUOTES, 'UTF-8');
                                    }
                                }
                            }
