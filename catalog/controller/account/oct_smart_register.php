    ...
    public function validate() {
        if (isset($this->request->server['HTTP_X_REQUESTED_WITH']) &&
            ...
            if (!$json) {
                ...
                $this->request->post['firstname'] = $this->sanitizeString($this->request->post['firstname'] ?? '', 32);
                $this->request->post['lastname'] = $this->sanitizeString($this->request->post['lastname'] ?? '', 32);
                $this->request->post['email'] = filter_var($this->request->post['email'] ?? '', FILTER_SANITIZE_EMAIL);
                $this->request->post['telephone'] = $this->request->post['telephone'] ?? '';
