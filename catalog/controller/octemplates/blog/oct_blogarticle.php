...
class ControllerOCTemplatesBlogOCTBlogArticle extends Controller {
	...
	public function write() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (!isset($json['error'])) {
				$blogarticle_id = isset($this->request->get['blogarticle_id']) ? (int)$this->request->get['blogarticle_id'] : 0;

				if ($blogarticle_id > 0) {
					$this->load->model('octemplates/blog/oct_blogcomment');

					$this->model_octemplates_blog_oct_blogcomment->addComment($blogarticle_id, $this->request->post);
					...
					if (isset($oct_sms_settings['templates'][$template_code]['status']) && $oct_sms_settings['templates'][$template_code]['status']) {
						...
						if (!empty($sms_template)) {
							...
							$replace = array(
								...
								'[post_link]' => $this->url->link('octemplates/blog/oct_blogarticle', 'blogarticle_id=' . $blogarticle_id),
