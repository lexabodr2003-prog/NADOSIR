...
class ControllerOctemplatesModuleOctOtpLogin extends Controller {
    ...
    public function sendOtp() {
        ...
        if ($this->request->server['REQUEST_METHOD'] === 'POST') {
            ...
            $this->session->data['otp_customer_id'] = (int)$customer_info['customer_id'];
            ...
            if ($this->isMaxAttemptsExceeded($customer_info['customer_id'])) {
                $otp_data = $this->model_octemplates_module_oct_otp_login->getOtp((int)$customer_info['customer_id']);
            ...
            $otp_code = $this->generateOtpCode((int)$customer_info['customer_id']);
    ...
    public function verifyOtp() {
        ...
        if ($otp_code_input_hashed !== $otp_data['otp_code']) {
            $this->model_octemplates_module_oct_otp_login->incrementAttempts((int)$customer_id);
            $logging = $moduleSettings['logging'] ?? false;
            $this->model_octemplates_module_oct_otp_login->logOtpAttempt((int)$customer_id, $telephone, 'failure', $logging);
            return $this->jsonResponse(['error' => $this->language->get('error_otp')]);
        }
    
        $this->model_octemplates_module_oct_otp_login->deleteOtp((int)$customer_id);
        $this->model_octemplates_module_oct_otp_login->resetLoginAttempts($ip_address);
        $this->model_octemplates_module_oct_otp_login->clearOldLoginAttempts();
    
        $this->loginCustomer((int)$customer_id);
        ...
        $logging = $moduleSettings['logging'] ?? false;
        $this->model_octemplates_module_oct_otp_login->logOtpAttempt((int)$customer_id, $telephone, 'success', $logging);
