    ...
    public function index() {
	    if ($this->config->get('oct_popup_found_cheaper_status') && isset($this->request->server['HTTP_X_REQUESTED_WITH']) && !empty($this->request->server['HTTP_X_REQUESTED_WITH']) && strtolower($this->request->server['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
			...
			if ($product_info) {
				...
				$data['ajax_send_url'] = str_replace('&amp;', '&', $this->url->link('octemplates/module/oct_popup_found_cheaper/send'));
	...
	public function send() {
		if ($this->config->get('oct_popup_found_cheaper_status') && isset($this->request->server['HTTP_X_REQUESTED_WITH']) && !empty($this->request->server['HTTP_X_REQUESTED_WITH']) && strtolower($this->request->server['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
			...
			if ($this->validate()) {
				...
				$pid_value = isset($this->request->post['pid']) ? strip_tags($this->request->post['pid']) : '';
				$mid_value = isset($this->request->post['mid']) ? strip_tags($this->request->post['mid']) : '';
				$linkgood = $pid_value . "\r\n" . $mid_value;
				...
				if (isset($oct_popup_found_cheaper_data['notify_status']) && $oct_popup_found_cheaper_data['notify_status']) {
					...
					if (isset($oct_sms_settings['templates'][$template_code]['status']) && $oct_sms_settings['templates'][$template_code]['status']) {
						...
						if (!empty($sms_template)) {
							$customer_name = '';
							if (isset($this->request->post['name'])) {
								$customer_name = strip_tags($this->request->post['name']);
								$customer_name = mb_substr($customer_name, 0, 50);
							}

							$telephone = '';
							if (isset($this->request->post['telephone'])) {
								$telephone = strip_tags($this->request->post['telephone']);
								$telephone = mb_substr($telephone, 0, 20);
							}

							$link = '';
							if (isset($this->request->post['link'])) {
								$link = filter_var($this->request->post['link'], FILTER_SANITIZE_URL);
							...
							$product_link = '';
							if (isset($this->request->post['pid'])) {
								$product_link = filter_var($this->request->post['pid'], FILTER_SANITIZE_URL);
								if (filter_var($product_link, FILTER_VALIDATE_URL)) {
									$product_link = mb_substr($product_link, 0, 100);
								} else {
									$product_link = strip_tags($this->request->post['pid']);
									$product_link = mb_substr($product_link, 0, 100);
								}
							}
