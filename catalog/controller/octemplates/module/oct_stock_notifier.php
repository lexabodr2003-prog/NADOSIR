...
class ControllerOctemplatesModuleOctStockNotifier extends Controller {
	...
	public function cron() {
		$config_data = $this->config->get('oct_stock_notifier_data');

		if (!$this->config->get('oct_stock_notifier_status') || empty($config_data['cron_secret'])) {
			$this->redirectToNotFound();
			return;
		}
		...
		if ($this->model_octemplates_module_oct_abandoned_cart->isRateLimited($ip, 'cron_stock_notifier', 5, 300)) {
			$this->redirectToNotFound();
			return;
		}

		$secret_key = $config_data['cron_secret'];
		$request_key = isset($this->request->get['cron_secret']) ? (string)$this->request->get['cron_secret'] : '';

		if (!$request_key || $request_key !== $secret_key) {
			if (!$this->model_octemplates_module_oct_abandoned_cart->checkRateLimit($ip, 'cron_stock_notifier', 5, 300)) {
				$this->log->write('[SECURITY] Multiple failed Cron key attempts (Stock Notifier) from IP: ' . $ip);
			}
	...
	private function notifySubscribers($subscribers, $product) {
		foreach ($subscribers as $subscriber) {
			...
			if (!empty($subscriber['phone'])) {
				...
				$language_id = (int)$subscriber['language_id'];
				...
				if (!empty($sms_template)) {
						$customer_name = strip_tags($subscriber['customer_name'] ?? '');
						$customer_name = mb_substr($customer_name, 0, 50);

						$product_name = isset($product['name']) && !empty($product['name']) ? strip_tags($product['name']) : 'Product';
						$product_name = mb_substr($product_name, 0, 100);

						$product_link = $this->url->link('product/product', 'product_id=' . (int)$product['product_id']);
	...
	private function prepareMailData($subscriber, $product) {
		...
		$email_data['product_name'] = htmlspecialchars($subscriber['name'], ENT_QUOTES, 'UTF-8');
		$email_data['store_name'] = htmlspecialchars($this->config->get('config_name'), ENT_QUOTES, 'UTF-8');
		$email_data['customer_name'] = htmlspecialchars($subscriber['customer_name'], ENT_QUOTES, 'UTF-8');
		...
		if (isset($module_data['custom_message']) && $module_data['custom_message'] == "on") {
			...
			$email_data['subject'] = sprintf($this->language->get('mail_subject'), $email_data['store_name'], $email_data['product_name']);
			$email_data['welcome'] = sprintf($this->language->get('mail_welcome'), $email_data['store_name']);
			$email_data['body'] = sprintf($this->language->get('mail_body'), $email_data['product_name'], $email_data['product_link']);
		}
		...
		return [
			...
			'storeName' => $email_data['store_name'],
			'customerName' => $email_data['customer_name'],
			'productName' => $email_data['product_name'],
	...
	private function notifyAdmin($data) {
		...
		$storeName = $this->config->get('config_name');
		$mailData = array(
			'welcome' => htmlspecialchars(sprintf($this->language->get('mail_admin_welcome'), html_entity_decode($storeName, ENT_QUOTES, 'UTF-8')), ENT_QUOTES, 'UTF-8'),
			'subject' => htmlspecialchars(sprintf($this->language->get('mail_admin_subject'), html_entity_decode($storeName, ENT_QUOTES, 'UTF-8'), $product_info['name']), ENT_QUOTES, 'UTF-8'),
			'body' => htmlspecialchars(sprintf($this->language->get('mail_admin_body'), $product_info['name'], $this->url->link('product/product', 'product_id=' . $data['pid'])), ENT_QUOTES, 'UTF-8'),
			'productName' => htmlspecialchars($product_info['name'], ENT_QUOTES, 'UTF-8'),
			'storeName' => htmlspecialchars($storeName, ENT_QUOTES, 'UTF-8'),
			'type' => 0
		);
		...
		if (isset($oct_sms_settings['templates'][$template_code]['status']) && $oct_sms_settings['templates'][$template_code]['status']) {
			...
			if (!empty($sms_template)) {
				$customer_name = strip_tags($data['name']);
				$customer_name = mb_substr($customer_name, 0, 50);
				...
				if (!empty($oct_sms_settings['admin_phone']) && !empty($oct_sms_settings['oct_sms_token'])) {
					$this->load->model('octemplates/module/oct_sms_notify');
	...
	private function sanitizeString($input, $max_length = 255) {
	...
	protected function validate() {
		...
		if (isset($this->request->post['name'])) {
			$this->request->post['name'] = $this->sanitizeString($this->request->post['name']);
		}
